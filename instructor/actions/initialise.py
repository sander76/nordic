            ...
            PvKeypadAlt(
                ...
                okay=Commands(
                    nordic_commands=NordicCommands(
                        Nd.RESET,
                        DelayedCommand(
                            Nd.M25S_DUETTE_TENSIONED, PRODUCT_SET_DELAY
                        ),
                        DelayedCommand(
                            Nd.ORIENT_M25S_DUETTE_RIGHT, PRODUCT_SET_DELAY
                        ),
                    ),
                    confirm_command=Confirm(
...
initialise_pleated = get_initialise(PLEATED_JOG_2, Nd.M25S_PLEATED_FREE)

initialise_pleated_tensioned = get_initialise(
    PLEATED_JOG_2, Nd.M25S_PLEATED_TENSIONED
)

initialise_vvb_right = get_initialise(VVB_JOG_1, Nd.M25S_VVB_RIGHT_STACK)

initialise_vvb_left = get_initialise(VVB_JOG_1, Nd.M25S_VVB_LEFT_STACK)

initialise_vvb_center = get_initialise(VVB_JOG_1, Nd.M25S_VVB_CENTER_STACK)
