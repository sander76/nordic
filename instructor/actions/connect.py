...
from instructor.constants import (
    ...
    VVB_JOG_1,
    VVB_BUTTON_PUSH_HOLD,
    VVB_BUTTON_RELEASE,
    PLEATED_JOG_1,
    PLEATED_BUTTON_PUSH_HOLD,
    PLEATED_BUTTON_RELEASE,
)
from server.nordic import Nd

connect_text = Row(
    Text(25, Tr.PRESS_HOLD_BLIND_BUTTON.add_number(1)),
...
def get_connect(jog_image, button_push_hold_image, button_release_image):
    return Step(
        ...
        [
            connect_text,
            Row(
                Image(25, button_push_hold_image),
                PvKeypadAlt(
                    25,
                    okay=Commands(
                        nordic_commands=NordicCommands(
                            Nd.NETWORKADD, DelayedCommand(Nd.GROUP_ADD, 1)
                        ),
...
connect_m25s_vb = get_connect(VB_JOG_1, VB_BUTTON_PUSH_HOLD, VB_BUTTON_RELEASE)
